...
```{r}
#| eval: false
#| label: Installation cruncher R packages

install.packages(c("rjwsacruncher", "JDCruncheR"))
```
...
```{r}
#| eval: false
#| label: Chargement cruncher R packages
...
```{r}
#| eval: false
#| label: Set option path cruncher

options(cruncher_bin_directory = "C:/Software/jwsacruncher-3.4.0/jdemetra-cli-3.4.0/bin")
```
...
what is not relevant in each case 
...
Some available output series will be different when using X-13-ARIMA or Tramo-Seats.


each concept : output name in outout chapter

Here: ow to get a given concetp 
...
Some parameters and available diagnostics will be different when using X-13-ARIMA or Tramo-Seats.
